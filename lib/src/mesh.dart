...
Future<List<Mesh>> loadObj(
    String fileName, bool normalized, bool isAssetFile) async {
  ...
  for (var line in lines) {
    ...
    switch (parts[0]) {
      case 'mtllib':
        ...
        materials = await loadMtl(mtlFileName, isAssetFile);
        break;
      case 'usemtl':
...
void _remapZeroAreaUVs(List<Offset> texcoords, List<Polygon> textureIndices,
  ...
  for (int index = 0; index < textureIndices.length; index++) {
    ...
    if (p.vertex0 < 0) p.vertex0 = p.vertex0 * -1;
    if (p.vertex1 < 0) p.vertex1 = p.vertex1 * -1;
    if (p.vertex2 < 0) p.vertex2 = p.vertex2 * -1;
...
void _rebuildVertices(List<Vector3> vertices, List<Offset> texcoords,
  ...
  for (int i = 0; i < vertexIndices.length; i++) {
    ...
    for (int j = 0; j < vi.length; j++) {
      int vIndex = vi[j];
      int tIndex = ti[j];
      if (vIndex < 0) vIndex = vIndex * -1;
      if (tIndex < 0) tIndex = tIndex * -1;
